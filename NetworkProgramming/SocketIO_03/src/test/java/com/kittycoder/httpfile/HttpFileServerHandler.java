...
/**
 * Created by shucheng on 2019-10-1 下午 22:01
 */
public class HttpFileServerHandler  extends SimpleChannelInboundHandler<FullHttpRequest> {
    ...
    protected void messageReceived(ChannelHandlerContext ctx, FullHttpRequest request) throws Exception {
        ...
        if (file.isDirectory()) {
            if (uri.endsWith("/")) {
                // 如果以正常“/”结束，说明访问的是一个文件目录：则展示文件列表（web服务端则可以跳转一个Controller，遍历文件并跳转到一个页面）
        ...
        sendFileFuture.addListener(new ChannelProgressiveFutureListener() {
            ...
            public void operationProgressed(ChannelProgressiveFuture future, long progress, long total) throws Exception {
                if (total < 0) { // total unknown
                    ...
                    System.err.println("Transer progress: " + progress + "/" + total);
                }
            }
        ...
        // 如果使用Chunked编码，最后则需要发送一个编码结束的空消息体，进行标记，标识所有消息体已经成功发送完成
    ...
    private String sanitizeUri(String uri) {
        ...
        // 当前工程所在目录 + URI构造绝对路径进行返回（这种方式在单模块下可以，多模块下行不通）
    ...
    private static void sendListing(ChannelHandlerContext ctx, File dir) {
        ...
        // 重置写出区域
        buffer.release();
        // 使用ctx对象写出并且刷新到SocketChannel中，并主动关闭连接（这里是指关闭处理发送数据的线程连接）
